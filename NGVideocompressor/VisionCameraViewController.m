...
@end
@interface VisionCameraViewController ()<UIGestureRecognizerDelegate,
PBJVisionDelegate,
...
- (void)viewDidLoad {
    ...
    strobeFrame.origin = CGPointMake(self.view.frame.size.width/2, 15.0f);
    ...
    CGRect doneframe = CGRectMake(0, 0, 50, 25);
    doneframe.origin =  CGPointMake(15.0f, 15.0f);
    done.frame = doneframe;
    [done addTarget:self action:@selector(closeClick) forControlEvents:UIControlEventTouchUpInside];
...
- (void)vision:(PBJVision *)vision capturedVideo:(NSDictionary *)videoDict error:(NSError *)error
{
    ...
    NSString *path = [documentsDirectory stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.mp4",[self createRandomURLStreamName]]];
    ...
    if(error){
        NSLog(@"\nPBJVisionCapturedVideoError: %@",error.localizedDescription);
        if(self.delegate && [self.delegate respondsToSelector:@selector(customCameraDidEndCapturingVideo:error:)]){
            [self dismissViewControllerAnimated:YES completion:^{
                [self.delegate customCameraDidEndCapturingVideo:nil error:error];
            }];
        }
        return;
    }else{
        ...
        if(self.delegate && [self.delegate respondsToSelector:@selector(customCameraDidEndCapturingVideo:error:)]){
            [self dismissViewControllerAnimated:YES completion:^{
                [self.delegate customCameraDidEndCapturingVideo:[NSURL fileURLWithPath:path] error:error];
            }];
        }
    }
